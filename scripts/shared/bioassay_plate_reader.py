...
def _scaled_plate_values(df: pd.DataFrame, *, control_wells: Sequence[str], scale_mode: str) -> pd.DataFrame:
    ...
    if mode in {"minmax_0_100", "positive_control_then_minmax_0_100", "control_then_minmax_0_100"}:
        ...
        sample_mask = ~out["is_positive_control"] if controls else pd.Series(True, index=out.index)
...
def read_plate_replicates(
    ...
    *,
    ...
    for index, file_path in enumerate(files, start=1):
        ...
        scaled = _scaled_plate_values(long_df, control_wells=plate_controls, scale_mode=scale_mode)
