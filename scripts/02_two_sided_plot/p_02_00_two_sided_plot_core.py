    ...
    *,
    ...
    if str(settings.input_type or "table").strip().lower() == "plate_reader":
        if plate_reader is None:
            raise ImportError("Plate-reader activity input requires support/bioassay_plate_reader.py.")
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--filtered", help="Path to <sample>_filtered_feature_table.csv from script 01.")
