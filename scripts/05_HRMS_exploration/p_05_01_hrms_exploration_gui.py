#!/usr/bin/env python3
"""CustomTkinter GUI for HRMS feature-table preparation and exploration."""
...
try:
    from p_05_00_hrms_exploration_core import read_table, run_pipeline, template_config  # type: ignore
except Exception as e:  # pragma: no cover
    raise SystemExit(
        "ERROR: Could not import the HRMS exploration core script.\n"
        "Place this GUI script in the same folder as p_05_00_hrms_exploration_core.py.\n\n"
        f"Details: {e}"
    )
...
def main() -> int:
    ...
    APP_STATE_FILE = _THIS_DIR / ".p_05_01_hrms_exploration_gui_state.json"
    ...
    var_output_dir = tk.StringVar(value=app_state.get("output_dir", str(_THIS_DIR / "Outputs_hrms_exploration")))
    ...
    def default_final_feature_table_path() -> str:
        out = Path(var_output_dir.get().strip().strip('"') or (_THIS_DIR / "Outputs_hrms_exploration")).expanduser()
    ...
    def validate_and_build_config() -> dict[str, Any]:
        output_dir = Path(var_output_dir.get().strip().strip('"') or (_THIS_DIR / "Outputs_hrms_exploration")).expanduser()
    ...
    def save_config_file() -> None:
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile="hrms_exploration_config.json",
        )
        if path:
    ...
    def apply_config(config: dict[str, Any]) -> None:
        var_output_dir.set(str(config.get("output_dir", "Outputs_hrms_exploration")))
    ...
    def save_template_file() -> None:
        path = filedialog.asksaveasfilename(
            ...
            defaultextension=".json",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")],
            initialfile="hrms_exploration_template.json",
        )
        if path:
