#!/usr/bin/env python3
"""CustomTkinter GUI for script 3 two-sided chromatogram plots."""
...
try:
    import p_03_00_two_sided_plot_core as core
...
def main() -> int:
    ...
    state_file = _THIS_DIR / ".p_03_01_two_sided_plot_gui_state.json"
    ...
    var_display_mode = tk.StringVar(value=str(activity_state.get("display_mode", "raw")))
    ...
    class ToolTip:
        ...
        def schedule_show(self, _event: Any = None) -> None:
            ...
            if self.frame is None and self._show_after is None:
        ...
        def show(self) -> None:
            ...
            self.frame = ctk.CTkFrame(root, fg_color="#111318", border_color=colors["border"], border_width=1, corner_radius=16, width=self.tooltip_width)
    ...
    def current_config() -> dict[str, Any]:
        return {
            ...
            "activity_table": {
                ...
                "display_mode": var_display_mode.get().strip(),
                "label": var_activity_label.get().strip() or "Activity",
            },
            "plot": {
    ...
    def build_activity_settings() -> core.ActivityTableSettings | None:
        ...
        return core.ActivityTableSettings(
            ...
            display_mode=var_display_mode.get().strip() or "raw",
            label=var_activity_label.get().strip() or "Activity",
        )
    ...
    def validate_common_inputs() -> dict[str, Any]:
        ...
        if not filtered.exists():
            raise ValueError("Choose an existing filtered CSV from script 2.")
        outdir = Path(var_outdir.get().strip() or (_THIS_DIR / "outputs")).expanduser()
    ...
    def apply_config(config: dict[str, Any]) -> None:
        ...
        var_display_mode.set(str(activity.get("display_mode", "raw")))
    ...
    file_row(card, 1, "Filtered fraction table", var_filtered, lambda: browse_file(var_filtered, "Select filtered feature CSV", csv_patterns), "CSV produced by script 2. It should contain the retained fraction features with fraction index, retention time, and area columns.")
    ...
    labeled_widget(card, "Number of fractions", make_entry(card, var_n_fractions, "96"), 1, 0, help_text="Total number of uniform collected fractions. This should match the fractionation method and the indices in the script 2 output.")
    ...
    card = make_card(activity_scroll, "Activity table")
    file_row(card, 0, "Activity table", var_activity_path, lambda: browse_file(var_activity_path, "Select activity table", table_patterns), "Optional table for the fraction bioassay overlay. One row should represent one fraction unless that row is a control row you exclude from plotting.", load_cmd=load_activity_table)
    labeled_widget(card, "Excel sheet", make_combo(card, var_sheet_name, [""]), 1, 0, help_text="For Excel workbooks, choose the worksheet containing the fraction assay data. CSV/TSV files do not use this field.")
    sheet_combo = card.grid_slaves(row=1, column=1)[0]
    sheet_combo.configure(command=reload_selected_sheet)

    card = make_card(activity_scroll, "Map activity columns")
    ...
    card = make_card(activity_scroll, "Controls and activity scaling")
    ...
    labeled_widget(card, "Display transform", make_combo(card, var_display_mode, ["raw", "percent_of_max", "inhibition_from_max"]), 2, 0, help_text="raw plots the processed values directly. percent_of_max rescales the largest plotted value to 100. inhibition_from_max plots 100 - percent_of_max, useful when lower assay signal means stronger activity.")
    labeled_widget(card, "Overlay label", make_entry(card, var_activity_label, "Normalized activity (%)"), 3, 0, help_text="Axis label used for the right-side activity bars.", colspan=3)
    ...
    card = make_card(activity_scroll, "Preview loaded activity data")
